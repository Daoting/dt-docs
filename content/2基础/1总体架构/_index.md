...
## 总体架构
本系统未严格按照某种具体的服务架构进行设计，[参见常见服务架构](#常见服务架构)，而是根据自身整体架构的特点，借鉴以上各架构的思想，比如DDD模式的领域事件、充血模式、领域服务，CQRS架构的读写分离，六边形架构的微服务API设计等，结合客户端xaml的优势，使用MVVM(Model-View-ViewModel)模式开发客户端控件，**实现客户端领域层和服务端领域层代码基本相同**，既可以将业务放在客户端处理也可以稍加修改放在服务端处理，完美实现技术与业务解耦、业务逻辑解耦、业务封装复用等关键功能，为开发人员提供一种易懂易用的框架风格。总体框架图：
![](6.png "总体架构")
...
### 领域层
领域层是业务开发过程中的重要部分，包括实体、实体读写、业务逻辑、领域服务等。但系统未采用DDD模式的聚合根、值对象、仓库等概念。
...
`Api`层是客户端提交请求的入口，对应于领域层的领域服务，`Api`层在实现上支持两种类型的请求。

#### RpcApi
一种类似于传统的远程过程调用RPC(Remote Procedure Call)，外部可通过`HTTP POST`方式向服务的`/.c`发送`json消息`，服务会将`json消息`转换为对RpcApi的调用。

类需要继承自`RpcApi`并带有Api标签，类中的每个公共方法就是一个RPC，方法的参数和返回结果等同于请求的内容和结果，这些都为数据，适用于数据访问、业务逻辑处理等内容，因采用富客户端单页模式，所以请求参数和返回结果都为数据，无UI相关内容，可以包含参数校验、实体加载、多种类型实体之间的业务交互、数据持久化等内容。如下图：
...
#### RouteApi
另一种是请求路由Api，外部可通过`HTTP POST`方式向服务的`不同路由`发送完全`自定义格式`的消息，服务会根据`不同的请求路由`转换为对`RouteApi`的调用，请求消息和URL的查询参数会作为输入参数传递给`RouteApi`。

类需要继承自`RouteApi`并带有Route标签。
{{< highlight cs >}}
[Route("/.test/普通路由")]
