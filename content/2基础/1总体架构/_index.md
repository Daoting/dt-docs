...
`Api`层不再作为完整的层存在，传统`Api`层对应于当前系统的领域服务，是客户端提交请求的入口，因采用富客户端单页模式，所以请求参数和返回结果都为数据，无UI相关内容，可以包含参数校验、实体加载、多种类型实体之间的业务交互、数据持久化等内容。在实现上`Api`类继承自`RpcApi`并带有Api标签，如下图：
![](22.png)
